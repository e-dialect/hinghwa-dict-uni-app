<template>
  <view>
    ...
    <view class="flex padding solid-bottom">
      ...
      <view class="flex-sub">
        ...
        <view class="text-grey text-df margin-top-xs">
          来源：
          <text :data-id="word.contributor.id" class="text-blue" @tap="toContributor">
    ...
    <!--标签选择-->
    <scroll-view class="bg-white nav text-center" scroll-x>
      <view
    ...
    <!--标签内容-->
    <swiper :current="tabIndex" class="screen-swiper" duration="500" style="height: 150vh" @change="tabSlide">
      <!--释义-->
      <swiper-item class="margin">
        <view v-for="(item, index) in definition" :key="index" class="margin-top-sm solid-bottom" style="width: 92vw">
          <view class="text-bold text-xl">{{ item.content }}</view>
          <view v-for="(item, index1) in item.example" :key="index1" class="padding-sm">
      ...
      <!--发音-->
      <swiper-item>
        <view v-if="pronunciation.length===0" class="margin">
          <text class="text-grey">该词语暂无语音哦～点击右下方贡献语音～</text>
        </view>
        <view v-for="(item, index) in pronunciation" :key="index" class="padding solid-bottom">
          <view class="flex justify-between align-center">
            <view>
              <view>
                <text class="text-bold">{{ item.pronunciation.pinyin }}</text>
                <text class="text-grey">{{ ` /${item.pronunciation.ipa}/ ` }}</text>
                <text class="cuIcon-notificationfill text-blue" @tap="playAudio(item.pronunciation.source)"></text>
              </view>
              <view class="margin-top">
                <text>来源：</text>
                <text class="text-blue" @tap="toContributor(item.contributor.id)">
