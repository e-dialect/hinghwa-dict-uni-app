<template>
    <view>
        ...
        <view v-if="publish_articles.length === 0" class="text-lg margin">
        	<text>这里暂时空空如也~</text>
        </view>
        <view v-else class="cu-card article no-card" @tap="article" :data-index="index" v-for="(item, index) in publish_articles" :key="index">
